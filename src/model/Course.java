...
public class Course {
    ...
    public static Course fromJson(JSONObject o) {
        Course c = new Course(o.getString("courseId"), o.getString("title"), o.getString("description"), o.getString("instructorId"));
        if (o.has("lessons")) {
            JSONArray la = o.getJSONArray("lessons");
            for (int i = 0; i < la.length(); i++) c.getLessons().add(Lesson.fromJson(la.getJSONObject(i)));
        }
        if (o.has("students")) {
            JSONArray sa = o.getJSONArray("students");
            for (int i = 0; i < sa.length(); i++) c.getStudents().add(sa.getString(i));
        }
        return c;
    }
}
